...
class AutoProcessUI(QDialog):
    ...
    def autoProcessButton(self):
        try:
            files_list = os.listdir(self.fromFolderEntry.text())
            files_list_len = len(files_list)

            process_no_list = [self.autoConvert3DEP, self.autoCreateCentralized]
            processes_no = 1
            current_process = 0
            for i in process_no_list:
                if i.isChecked():
                    processes_no += 1

            self.autoProcessProgressbarMacro.setRange(0, processes_no)
            self.autoProcessProgressbarMacro.setValue(0)


        ##  Convert 3DEP data to normal data   ##
            autoProcessConvert3DEP(autoProcessUI)

            if process_no_list[0].isChecked():
                current_process += 1
            self.autoProcessProgressbarMacro.setValue(current_process)

        ##  Load, fit and save processed data ##
            autoProcessLoadandFit(autoProcessUI, datapointsUI, window, data, fit)

            current_process += 1
            self.autoProcessProgressbarMacro.setValue(current_process)


        ##  Centralize data in a new excel data ##
            autoProcessSaveData(autoProcessUI, window, data, fit)

            if process_no_list[1].isChecked():
                current_process += 1
            self.autoProcessProgressbarMacro.setValue(current_process)
        except:
...
#Initiation of GUI
    def __init__(self):
        QMainWindow.__init__(self)
        loadUi("DEP_UI.ui", self)
        self.setWindowTitle("DEP plotter")
        self.setWindowIcon(QtGui.QIcon("Logo.png"))
...
# Activating buttons #
    ...
    def updatingElectricField(self):
        if self.fitting_gen_fieldgrad_comboBox.currentText() == 'Other':
        ...
        elif self.fitting_gen_fieldgrad_comboBox.currentText() == '3DEP':
...
# Exiting the program #
    def exitButton(self):
        hopa_params.refreshFromGUIFittingParameters(paramsUI, window, 'hopa')
        sish_params.refreshFromGUIFittingParameters(paramsUI, window, 'sish')
        hopa_params.saveParameters('default_hopa_parameters')
        sish_params.saveParameters('default_sish_parameters')
        app.exit()
...
# Fits the loded/ploted data with the model selected #
    ...
    # Data fiting #
        ...
        if self.fitting_gen_fieldgrad_comboBox.currentText() == '3DEP':
            instanced_list = []
...
hopa_params.startupParameters('default_hopa_parameters')
hopa_params.changeGUIFittingParameters(paramsUI, window, 'hopa')

sish_params = MyParameters()
sish_params.startupParameters('default_sish_parameters')
sish_params.changeGUIFittingParameters(paramsUI, window, 'sish')
